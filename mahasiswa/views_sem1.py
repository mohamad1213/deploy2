...
def index_staf(req):
    ...
    if req.POST:
        ...
        if form_input.is_valid():
            ...
            form_input.save()
            messages.success(req, 'Data telah ditambahkan.')
        return redirect('/staf_sem1')
        

    group = req.user.groups.first()
    if group is not None and group.name == 'staf':
        tasks = models.Pkl.objects.all()
    return render(req, 'staf_sem1/index.html',{
        'data': tasks,
...
def detail(req, id):
    pkl = models.Pkl.objects.filter(pk=id).first()    
    return render(req, 'sem1/detail.html', {
        'data': pkl,
    })
...
def detail_staf(req, id):
    pkl = models.Pkl.objects.filter(pk=id).first()    
    return render(req, 'staf_sem1/detail.html', {
        'data': pkl,
    })
...
def update(req, id):
    ...
    pkl = models.Pkl.objects.filter(pk=id).first()    
    return render(req, 'sem1/update.html', {
        'data': pkl,
    })
...
def update_staf(req, id):
    ...
    pkl = models.Pkl.objects.filter(pk=id).first()    
    return render(req, 'staf_sem1/update.html', {
        'data': pkl,
    })
